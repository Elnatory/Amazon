...
const filteredProducts2 = currentProducts
  ...
  .filter(product => {
    // Convert the provided brand names to lowercase for case-insensitive comparison
    const brandNames = name.split(',').map(brand => brand.trim().toLowerCase());

    // Check if any subcategory name includes any of the provided brand names
    return product.subcategory.some(sub => {
      return (
        typeof sub.name === 'string' &&
        brandNames.some(brand => sub.name.toLowerCase().includes(brand))
      );
    });
  });
...
<div className='container '>
      ...
      <div className='row flex  catbackground'>
        <div className="col-lg-2   mx-2">
          <div className="text-lg font-bold mt-3 ">Any Department </div>
          {subCategories?.map((subcategorie) =>
            <div key={subcategorie._id} className="">
              <div className='w-full column p-3 border-2 hover:text-orange-600 mb-3'>
